...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Harvi Code and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Harvi Code) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Harvi Code
...
- Single-client scenarios (one Harvi Code instance per server)
...
1. The client (Harvi Code) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Harvi Code
...
STDIO servers run locally on the same machine as Harvi Code, which has several important implications:
...
- **Execution**: Starts and stops with Harvi Code (child process lifecycle)
...
- Start when needed by Harvi Code
- Not require network configuration
- Need to be installed alongside Harvi Code or via a package manager
...
## Configuring Transports in Harvi Code

For detailed information on configuring STDIO and SSE transports in Harvi Code, including example configurations, see the [Understanding Transport Types](/features/mcp/using-mcp-in-kilo-code#understanding-transport-types) section in the Using MCP in Harvi Code guide.
